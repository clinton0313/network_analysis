...
#Print statement
for lamb in [1, 2]:
    optim = optimal_seed_results[optimal_seed_results["Lambda"] == lamb]
    optim = optim[optim["Diffusion"] == optim["Diffusion"].max()]
...
ax.scatter(x = [res[1], res[2]], 
    ...
    color="blue", marker="D", label="Optimal Seeds (lambda=2)")
ax.scatter(x = [14, 25], 
    y = [eigen[14.], eigen[25]],
    color="red", marker="D", label="Optimal Seeds (lambda=1)")
ax.legend()
ax.set_xlabel("Seeds")
...
axb.scatter(x = [res[1], res[2]], 
    y = [between[res[1]], between[res[2]]],
    color="powderblue", marker="D", label="Optimal Seeds(lambda=2)")
axb.scatter(x = [14, 25], 
    y = [between[14.], between[25]],
    color="red", marker="D", label="Optimal Seeds (lambda=1)")
axb.legend()
...
network.set_model(seed_model)
network.set_strategy(optimal_seeding)
network.simulate(max_criminals = 1000, max_investigations = 6)
network.plot()
